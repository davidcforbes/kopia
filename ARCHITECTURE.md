...
> *target* is the **RustBack** 5-executable topology — `rustback-server`
> + `rustback` + `rustback-blockcopy` + `rustback-filecopy` +
> `rustback-mirror` — defined canonically in
> [`architecture-vision.md` §5.7](architecture-vision.md). `rustback-dump`
> and `rustback-indexer` fold into `rustback-server` as modules; the
...
| Is the D: → E: replica fresh and healthy?          | `rustback-dump.exe` STATUS CARDS "Replica:" row (kopia-iwz). Underlying inputs: presence of `C:\dev\kopia\logs\BACKUP_REPLICA_FAIL.flag` (daily failed) **or** `C:\dev\kopia\logs\BACKUP_REPLICA_VERIFY_FAIL.flag` (weekly verify caught corruption) — either flag → Failed. Otherwise the verdict comes from the newest `replica summary` line in `daily_kopia.log`: PASS if errors=0 and ≤30 h old; STALE if no summary in 30 h. For "is the replica running RIGHT NOW", check `Get-Process rustback-mirror` and tail `C:\dev\kopia\logs\daily_d_replica.log` for `[mirror]` lines (one per subtree start/end + a `[mirror] [<subtree>] rc=N duration=Ns bytes_written=N files=N chunks_changed=N/N` per completion). |
...
| `C:\dev\kopia\logs\daily_kopia.log` (additional `replica summary` line) | `daily_d_replica.ps1` finally block | Each replica run appends one `replica summary bytes=N chunks_changed=N chunks_total=N duration_s=Ns errors=N files=N mode=NORMAL\|DRY-RUN robocopy_rc=N shadow_id={...} source=D: target=E: tool=rustback-mirror` line. `rustback`/`rustback-dump` parse this to populate the Replica STATUS CARD (kopia-iwz). `robocopy_rc` is the legacy field name for the worst exit code across all rustback-mirror invocations (0 = all subtrees PASS; non-zero = at least one subtree failed); name preserved for rustback-dump compat (kopia-8mz tracks the rename). `chunks_changed/chunks_total` summarize the chunk-CBT delta across all subtrees. Retired fields (pre-2026-05-13): `rsync_kopia_rc`, `rsync_wbadmin_rc`, `link_dest_used`, `tool=rsync`. |
...
## Performance optimization landscape (rustback-mirror)
...
manifest's recorded `last_usn`, `rustback-mirror` skips the disk read and
...
- `rustback-mirror summary mode=cbt ... chunks_skipped_usn=N usn_state=...`
  on stdout
