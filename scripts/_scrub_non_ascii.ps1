...
foreach ($f in $files) {
    ...
    if ($changed -and -not $WhatIf) {
        # Pre-write parse-check (kopia-02h). Past brace-eating incidents
        # (2026-05-12, 2026-05-14) made scheduled tasks fail at parse time
        # with zero log output and a Valid-but-broken signature. Refuse to
        # overwrite the original file if the scrubbed text fails AST parse.
        $errs = $null; $null = [System.Management.Automation.Language.Parser]::ParseInput($text, [ref]$null, [ref]$errs)
        if ($errs -and $errs.Count -gt 0) {
            $first = $errs[0]
            throw "REFUSING TO WRITE: scrubbed $($f.Name) fails AST parse ($($errs.Count) errors; first at $($first.Extent.StartLineNumber):$($first.Extent.StartColumnNumber): $($first.Message)) -- original file preserved"
        }
        $tmp = "$($f.FullName).new"
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBoWBwa07J6wgtn
# Tnc2jvTLxZDZ2tL5/eBkObHZKV0ZRKCCIjAwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# AQkEMSIEIAbdNxb9ecyUnmLuZof3ZMbUR/e6zulH2BJH52AtdGDpMA0GCSqGSIb3
# DQEBAQUABIIBgCeKdTuq6JnCBuaVkQgs2ithFs39NZ2GcQeOBykFhX4IMqYl6AFo
# xRjzqn7cc4jyN2J5uXIOz/OnhhaAVG1pxLVa/o8+tIQx2IOd56SA/GW8DHR4tMgs
# O6MLpMC5PZ0dBQxrTNUd99y4P2Lp6CS+yLSL/S7x+qIntjb/+sAYtt8zWbrkjSSz
# eEPY5nrV/2ppH5SY53Uf0atrWnx7F82PnZ+yxyQxHhJlQrvX2P5SvTqCk3NuCTJr
# xrjZPEYm7Sc8sxD9UR4KyzGwcxywX3mzb1QxJ+CSEE3EXS6nAXpiYt3nozNw006s
# spopiCvZQSkflPGgcMnao6ckV3FcSH28IRTt9ZWqFQi2EPtkaRrvyZYtRCg8I6W3
# qg/R+t+J6bCkEsw/yQeqxXmfeprtVwLA3c6dGjgQ5tuNq8ntCtm5BNilSRDVQf6e
# pBH3HkuSvoIES5bUj9om0Nxe2OSn0F6E1U8jFGAyiT43RPD2riyotO2OLsip2ERZ
# CouAadMkUCjtgqGCGBQwghgQBgorBgEEAYI3AwMBMYIYADCCF/wGCSqGSIb3DQEH
# AqCCF+0wghfpAgEDMQ8wDQYJYIZIAWUDBAIBBQAwggFiBgsqhkiG9w0BCRABBKCC
# AVEEggFNMIIBSQIBAQYKKwYBBAGEWQoDATAxMA0GCWCGSAFlAwQCAQUABCCC0sK+
# B6O926RUZshRLdY6jqBuZha1hki0nphrY4nSYwIGaeiBKKSrGBMyMDI2MDUxNjE0
# NTcyOC42MjNaMASAAgH0oIHhpIHeMIHbMQswCQYDVQQGEwJVUzETMBEGA1UECBMK
...
# HAYJKoZIhvcNAQkFMQ8XDTI2MDUxNjE0NTcyOFowLwYJKoZIhvcNAQkEMSIEIOgo
# hXeHwu31g96EZ1SKIQ1K4edYQa4HF9xJnztZMOS4MIG5BgsqhkiG9w0BCRACLzGB
...
# AgC3+N7h3hhygmI7VA9cmDmzqDgcC+ceXku7iVtra6eNso/M0JpHBYzt6rT9WpxI
# MNhqMVlBRC42NS87wt/h6lv6vYFnhHivsK60QiUGcuSpd9iKStFfJthdY2kgfavc
# uTMM2zsk48/gfD0rFGngz18I6eZKaIw0qnU1NhuWeXwfpaI7K7MOk0zEwKyOUdx1
# XTrAiKICrzJcVYXYdqAvau9M3PsxiRJni2K7vKfjhz4wL4fVjbbnB4v14aBWiNcn
# 33SPPVnQMWcWr3fiTgk+FiXWuBNXf505VD97JD0G4Qf8kIHiV7+8xQzSA5UCrxxP
# DANflEwFkSjmSEZyftspkR7XvNu1iQPksO9N6pUgzEHK2Qnhtusk9pFHWAMMkHjc
# m3aTnv15yakkGaYgogeh6M95VjtVFlVfp+SPC7hFb2oa3ouM67TT6eZ+TFS/qVqz
# bgsOWtIaidLFugUWmjFdM/+uv+f2dhoVZM3xoMrvPiU9422gkyCLF6XGxZDf7YJm
# 1sSmG8PSnQ3CiXKsjO1J6fXEacW/py2t1xkVtbdDMb09ZzNuy/EJpWRU4R7XEVpR
# Yb9Kx01nxjCaJXo7sHdyByXDmZ4/1SjRCN+6WgHxeD0itowLnN1NxhIIqh/8vfpT
# CGQrxbnFNipDJuSakJn0PFv8h5yyC8D0vnpYAIltAtSVCA==
